...
--You can Tribute 1 Dragon monster and 1 LIGHT monster; Special Summon this card from your hand
function s.spcheck(sg,tp)
	return aux.ReleaseCheckMMZ(sg,tp) and sg:IsExists(s.chk,1,nil,sg)
end
function s.chk(c,sg)
	return c:IsRace(RACE_DRAGON) and c:IsOnField() and sg:IsExists(Card.IsAttribute,1,c,ATTRIBUTE_LIGHT)
end
function s.cfilter(c,tp)
	return (c:IsRace(RACE_DRAGON) or c:IsAttribute(ATTRIBUTE_LIGHT)) and c:IsOnField() 
end
