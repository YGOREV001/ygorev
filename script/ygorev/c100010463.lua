...
local s,id=GetID()
local YGOREV_FUSION_MAT1=100010007
local YGOREV_FUSION_MAT2=100010023
...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- Cannot be destroyed by battle, except by a WIND monster OR a Level 7 or higher monster
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e2:SetValue(function(_,c) return not (c:IsAttribute(ATTRIBUTE_WIND) or c:IsLevelAbove(7)) end)
	c:RegisterEffect(e2)
end
...
--Card Effects--START
--While your opponent does not control a WIND monster, this card can attack your opponent directly
function s.filter(c)
