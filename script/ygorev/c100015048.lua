--Sword Arm Stego
local s,id=GetID()
local YGOREV_FUSION_MAT1=100010148
local YGOREV_FUSION_MAT2=100010016
...
--Card Effects--START
--If a Dinosaur monster you control destroys an opponent's monster by battle
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	local bc=tc:GetBattleTarget()
	return #eg==1 and tc:IsReason(REASON_BATTLE) and bc:IsRelateToBattle() and bc:IsControler(tp) and bc:IsRace(RACE_DINOSAUR)
end
...
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (Duel.IsExistingMatchingCard(s.envfilter,e:GetHandlerPlayer(),LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) or Duel.IsEnvironment(YGOREV_CARD_WASTELAND)) then return end
