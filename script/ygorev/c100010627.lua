...
function s.filter1(c,tp)
	...
	return lv>=1 and c:IsType(TYPE_MONSTER) and c:IsDiscardable() and c:IsAbleToGraveAsCost()
		and Duel.IsExistingTarget(s.filter2,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,lv)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SelectTarget(tp,s.filter2,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil,lv)
