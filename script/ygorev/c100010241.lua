...
function s.initial_effect(c)
	--f an Insect monster you control is destroyed by battle
...
function s.cfilter(c,tp)
	return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp)
		and c:IsReason(REASON_BATTLE) and c:IsRace(RACE_INSECT)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Gains 100 ATK for each Insect monster curently in your Graveyard
