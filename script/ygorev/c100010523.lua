--Flame Ghost
local s,id=GetID()
local YGOREV_FUSION_MAT1=100010441
...
function s.damfilter(c)
	return (c:IsRace(RACE_ZOMBIE) or c:IsAttribute(ATTRIBUTE_FIRE)) and c:IsFaceup() and c:IsAbleToGrave()
end
function s.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local ct=Duel.GetMatchingGroupCount(s.damfilter,tp,LOCATION_MZONE,0,1,nil)
...
function s.atkfilter(c,e,tp)
	return c:IsCode(YGOREV_CARD_SKULLSERVANT) or c:IsAttribute(ATTRIBUTE_FIRE)
end
