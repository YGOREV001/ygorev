...
local YGOREV_FUSION_MAT2=100010220
...
function s.filter1(c,tp)
	local tpe=c:GetType()
	return tpe~=0 and c:IsAbleToGraveAsCost()
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_ONFIELD,0,1,nil,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	local cg=Duel.SelectMatchingCard(tp,s.filter1,tp,LOCATION_ONFIELD,0,1,1,nil,tp)
