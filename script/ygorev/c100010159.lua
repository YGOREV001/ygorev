...
--Gains 300 ATK/DEF for each Dragon or Rock monster on the field.
function s.atkfilter(c,e,tp)
	return c:IsRace(RACE_DRAGON+RACE_ROCK) and c:IsFaceup()
end
...
function s.filter(c,e,tp,df)
	return c:IsRace(RACE_DRAGON+RACE_ROCK) and c:IsDefenseBelow(df) and c:IsType(TYPE_NORMAL) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	local bc=c:GetBattleTarget()
	e:SetLabel(bc:GetDefense())
...
--Special Summon 1 Dragon or Rock Normal Monster from your Deck in Defense Position, whose DEF is lower or equal to the destroyed monster's DEF it had on the field
