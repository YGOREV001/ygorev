--Dark Ichthyocentaur
local s,id=GetID()
local YGOREV_FUSION_MAT1=100010233
local YGOREV_FUSION_MAT2=100010139
...
--Card Effects--START
--If a monster(s) is sent to the Graveyard: This card gains 100 ATK/DEF
...
--Once, while this card is face-up on the field, you can make this card's DEF becomes 0, and if you do, add 1 Spell from your Deck to your hand, except a Continuous OR Normal Spell
function s.thfilter(c,e,tp)
	return (c:IsType(TYPE_EQUIP+TYPE_QUICKPLAY+TYPE_FIELD+TYPE_RITUAL) and c:IsType(TYPE_SPELL)) and c:IsAbleToHand()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsFacedown() or not c:IsRelateToEffect(e) or c:GetDefense()==0 then return end
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
