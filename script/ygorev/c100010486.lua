--Metal Fish
local s,id=GetID()
local YGOREV_FUSION_MAT1=100010048
local YGOREV_FUSION_MAT2=100010234
...
--Card Effects--START
--Tribute this card; Special Summon 1 Level 6 or lower Machine or WATER monster from your Extra Deck, except "Metal Fish"
...
function s.spfilter(c,e,tp)
	return c:IsType(TYPE_FUSION) and (c:IsRace(RACE_MACHINE) or c:IsAttribute(ATTRIBUTE_WATER)) and (not c:IsCode(id)) and c:IsLevelBelow(6) and Duel.GetLocationCountFromEx(tp,tp,e:GetHandler(),c)>0 and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
